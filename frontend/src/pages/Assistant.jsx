...
export default function Assistant({ batteryId }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-120px)] md:h-[calc(100vh-160px)] animate-fade-in">
      {/* Header */}
      <div className="glass-card-amber p-3 md:p-4 mb-3 md:mb-4 flex items-center gap-2 md:gap-3">
        <div className="w-8 h-8 md:w-9 md:h-9 rounded-xl bg-amber-500/20 border border-amber-500/30 flex items-center justify-center shrink-0">
          <Sparkles size={15} className="text-amber-400" />
        </div>
        <div className="min-w-0 flex-1">
          <h2 className="text-xs md:text-sm font-semibold text-slate-200">BatteryGuard AI</h2>
          <p className="text-[10px] md:text-[11px] text-slate-500 truncate">
            LLaMA 3.1 70B · NVIDIA NIM + ChromaDB
            {batteryId && <span className="text-amber-600 ml-1">· {batteryId}</span>}
          </p>
        </div>
        <button onClick={() => setMessages([messages[0]])}
          className="ml-auto btn-ghost text-xs gap-1.5 py-1.5 px-2 md:px-3 shrink-0">
          <RefreshCw size={11} /> <span className="hidden sm:block">Clear</span>
        </button>
      </div>

      {/* Preset queries */}
      {messages.length <= 1 && (
        <div className="flex flex-col sm:flex-row flex-wrap gap-2 mb-3 md:mb-4">
          {PRESET_QUERIES.map((q, i) => (
            <button key={i} onClick={() => send(q)}
              className="text-[11px] px-3 py-2 sm:py-1.5 rounded-lg border border-white/8 text-slate-500
      ...
      <div className="mt-3 md:mt-4 flex gap-2">
        <input
          ...
          disabled={loading}
          className="flex-1 bg-white/5 border border-white/8 rounded-xl px-3 md:px-4 py-3 text-sm text-slate-200
                     placeholder:text-slate-700 focus:outline-none focus:border-amber-500/40
                     focus:bg-white/8 disabled:opacity-50 transition-all min-w-0"
        />
        <button
          onClick={() => send()}
          disabled={loading || !input.trim()}
          className="btn-primary disabled:opacity-40 disabled:cursor-not-allowed py-3 px-3 md:px-4 shrink-0"
        >
