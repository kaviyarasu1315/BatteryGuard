...
import { Battery, RefreshCw, Wifi, WifiOff, BookOpen, X } from 'lucide-react'
import BatteryCard from './BatteryCard'
import { fetchBatteries, healthCheck, fetchKnowledgeStatus, ingestKnowledge } from '../api/client'

export default function Sidebar({ selectedBattery, onSelectBattery, onClose, isMobileOpen }) {
  ...
  return (
    <aside className="w-72 md:w-64 shrink-0 flex flex-col h-full border-r border-white/5 bg-[#050d1a] md:bg-navy-900/60 backdrop-blur-xl">
      ...
      <div className="p-4 md:p-5 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div
              className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center shadow-lg"
              style={{ boxShadow: '0 4px 16px rgba(245,158,11,0.4)' }}
            >
              <Battery size={18} className="text-[#050d1a]" />
            </div>
            <div className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-emerald-400 rounded-full border-2 border-[#050d1a]" />
          </div>
          <div>
        ...
        <button
          onClick={onClose}
          className="md:hidden w-8 h-8 flex items-center justify-center rounded-xl border border-white/8 text-slate-400 hover:text-white hover:bg-white/5 transition-all"
        ...
        <div className="glass-card p-3 rounded-xl">
          ...
          {kbStatus && (
            <div className="text-[11px] text-slate-500 mb-2">
              {kbStatus.status === 'ready' && kbStatus.chunk_count > 0
